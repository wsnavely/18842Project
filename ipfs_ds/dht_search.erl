...
%% Currently max number of members in search list is 32
%% If we are almost done with the final search, we redo the search
%% for 4 times to confirm the end of search.
...
find(Type, ID) ->
    search_iterate(Type, ID, ?SEARCH_LIM, dht_state:closest_to(ID, ?SEARCH_LIM)).
...
%% search iteratively for the nearest nodes to an id
%% @param current node id, 
%%		  query type, 
%%		  target id,
%%		  number of nodes wants to be searched,
%%		  number of retries left
%%		  number of nodes which is the result returned by last search, and to be searched soon
%%		  number of nodes that has been searched, to evict duplicated search nodes
%%		  number of nodes that are alive
%%		  acc is only for find_value

%% recursion exit		  
dht_iter_search(_NodeID, find_node, _Target, _Width, 0, _Todo, _Done, Alive, _Acc) ->
    gb_sets:to_list(Alive);  %return alive nodes
